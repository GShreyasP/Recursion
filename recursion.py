...
def group_sum_5(start, nums, target):
    ...
    if start >= len(nums):
        return target == 0
    target -= nums[start]
    if (nums[start] % 5 == 0) and ((len(nums) - 1) >= (start + 1)) and nums[start + 1] == 1:
        if group_sum_5(start+2, nums, target):
            return True
    elif not nums[start] % 5 == 0:
